...
export default function Project() {
  ...
  useEffect(() => {
    fetch('api/json/project.json')
      ...
      .then((data) => setProjectData(Array.isArray(data?.projects) ? data.projects : []))
      .catch((error) => console.error('Error loading JSON:', error));
  }, []);

  useEffect(() => {
    const updateGridLimit = () => setGridLimit(window.innerWidth <= 640 ? MOBILE_GRID_LIMIT : GRID_LIMIT);
    updateGridLimit();
    window.addEventListener('resize', updateGridLimit);
    return () => window.removeEventListener('resize', updateGridLimit);
  }, []);
  ...
  return (
    <section id="projects" className="min-h-screen bg-primary flex flex-col items-center px-8 py-24 border-b-2">
      <h2 className="text-3xl font-bold text-textPrimary text-center">Projects</h2>
      <h3 className="text-2xl font-bold text-textPrimary pb-20 text-center">Some of my recent projects</h3>
            ...
            <div className="p-4 min-h-72">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex space-x-5">
                  {project.githubLink && (
                    <a href={project.githubLink} target="_blank" rel="noopener noreferrer">
                      <IconBrandGithub className="text-gray-600 w-6 h-6 hover:text-green-400" />
                    </a>
                  )}
                  {project.externalLink && (
                    <a href={project.externalLink} target="_blank" rel="noopener noreferrer">
                      <FaFolderOpen className="text-gray-600 w-6 h-6 hover:text-green-400" />
                    </a>
                  )}
                </div>
              </div>
              <p className="bg-slate-200 p-2 rounded-md">{project.description}</p>
            </div>
